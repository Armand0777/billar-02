  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-white flex items-center gap-2">
            <Trophy className="w-7 h-7 text-malandro-red" /> 
        ...
        <div className="flex items-center gap-3">
          ...
          <button 
            ...
            className="flex items-center gap-2 px-4 py-2 bg-malandro-red hover:bg-malandro-red-dark text-white rounded-xl text-sm font-bold transition-all shadow-[0_0_20px_rgba(211,47,47,0.3)]"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {loading ? (
          <div className="col-span-full py-12 text-center text-malandro-gray">Cargando torneos...</div>
        ) : campeonatos.length > 0 ? (
          campeonatos.map((camp) => (
            <div key={camp.id_campeonato} className="bg-[#1a1a1c] border border-[#2a2a2c] rounded-2xl p-6 hover:border-malandro-red/50 transition-all flex flex-col group">
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-3">
                  <div className={`p-3 rounded-xl ${camp.estado === "proximo" ? "bg-blue-500/10 text-blue-400" : camp.estado === "en_curso" ? "bg-green-500/10 text-green-400" : "bg-[#2a2a2c] text-malandro-gray"}`}>
                    <Trophy className="w-6 h-6" />
                  </div>
                  <div>
                    <h3 className="text-lg font-bold text-white leading-tight">{camp.nombre}</h3>
                    <span className="text-[10px] uppercase font-bold tracking-wider text-malandro-gray">{camp.estado.replace("_", " ")}</span>
                  </div>
                </div>
              </div>
              
              <div className="space-y-3 mb-6 flex-1">
                <div className="flex items-center gap-2 text-sm text-malandro-gray">
                  <Calendar className="w-4 h-4 text-malandro-red" />
                  <span>{new Date(camp.fecha_inicio).toLocaleDateString("es-BO")}</span>
                </div>
                <div className="flex items-center gap-2 text-sm text-malandro-gray">
                  <Users className="w-4 h-4 text-malandro-red" />
                ...
                {camp.premio && (
                  <p className="text-sm text-malandro-gray bg-[#2a2a2c]/50 p-2 rounded-lg italic">
                    Premio: {camp.premio}
                  </p>
                )}
              </div>
              ...
              <button 
                ...
                className="w-full flex items-center justify-between px-4 py-3 bg-[#2a2a2c] hover:bg-malandro-red text-white rounded-xl text-sm font-bold transition-all group-hover:shadow-[0_0_20px_rgba(211,47,47,0.2)]"
              >
          ...
          <div className="col-span-full py-12 text-center text-malandro-gray border border-dashed border-[#2a2a2c] rounded-2xl">
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#1a1a1c] border border-[#2a2a2c] w-full max-w-lg rounded-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-[#2a2a2c] flex justify-between items-center">
              ...
              <button onClick={() => setIsCreating(false)} className="p-2 hover:bg-[#2a2a2c] rounded-full text-malandro-gray"><X className="w-5 h-5" /></button>
            </div>
            <div className="p-6 space-y-4">
              <div>
                <label className="text-sm font-medium text-malandro-gray block mb-1">Nombre del Torneo *</label>
                <input type="text" value={formData.nombre} onChange={e => setFormData({...formData, nombre: e.target.value})} className="w-full bg-black/40 border border-[#2a2a2c] rounded-lg py-2.5 px-3 text-white focus:border-malandro-red outline-none" placeholder="Ej: Torneo Relámpago Bola 8" />
              </div>
              <div>
                <label className="text-sm font-medium text-malandro-gray block mb-1">Banner del Torneo (Opcional)</label>
                <ImageUploader 
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-sm font-medium text-malandro-gray block mb-1">Fecha Inicio *</label>
                  <input type="date" value={formData.fecha_inicio} onChange={e => setFormData({...formData, fecha_inicio: e.target.value})} className="w-full bg-black/40 border border-[#2a2a2c] rounded-lg py-2.5 px-3 text-white focus:border-malandro-red outline-none [color-scheme:dark]" />
                </div>
                <div>
                  <label className="text-sm font-medium text-malandro-gray block mb-1">Cupo Máximo</label>
                  <input type="number" value={formData.cupo_maximo} onChange={e => setFormData({...formData, cupo_maximo: e.target.value})} className="w-full bg-black/40 border border-[#2a2a2c] rounded-lg py-2.5 px-3 text-white focus:border-malandro-red outline-none" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-sm font-medium text-malandro-gray block mb-1">Costo Inscripción (Bs.)</label>
                  <input type="number" step="0.01" value={formData.precio_inscripcion} onChange={e => setFormData({...formData, precio_inscripcion: e.target.value})} className="w-full bg-black/40 border border-[#2a2a2c] rounded-lg py-2.5 px-3 text-white focus:border-malandro-red outline-none" />
                </div>
                <div>
                  <label className="text-sm font-medium text-malandro-gray block mb-1">Premio</label>
                  <input type="text" value={formData.premio} onChange={e => setFormData({...formData, premio: e.target.value})} className="w-full bg-black/40 border border-[#2a2a2c] rounded-lg py-2.5 px-3 text-white focus:border-malandro-red outline-none" placeholder="Ej: 1000 Bs + Trofeo" />
                </div>
              </div>
              <div>
                <label className="text-sm font-medium text-malandro-gray block mb-1">Descripción</label>
                <textarea rows={2} value={formData.descripcion} onChange={e => setFormData({...formData, descripcion: e.target.value})} className="w-full bg-black/40 border border-[#2a2a2c] rounded-lg py-2.5 px-3 text-white focus:border-malandro-red outline-none" placeholder="Reglas, formato..." />
              </div>
            </div>
            <div className="p-6 border-t border-[#2a2a2c] bg-black/20 flex gap-3">
              ...
              <button onClick={handleCreate} className="flex-1 py-2.5 rounded-lg bg-malandro-red hover:bg-malandro-red-dark text-white font-bold text-sm">Crear Torneo</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="bg-[#1a1a1c] border border-[#2a2a2c] w-full max-w-2xl rounded-2xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
            <div className="p-6 border-b border-[#2a2a2c] flex justify-between items-center shrink-0">
              <div>
                ...
                <p className="text-xs text-malandro-red font-bold uppercase">{activeCampeonato.nombre}</p>
              </div>
              <button onClick={() => setIsRegistering(false)} className="p-2 hover:bg-[#2a2a2c] rounded-full text-malandro-gray"><X className="w-5 h-5" /></button>
            </div>
            ...
            <div className="p-6 bg-black/20 border-b border-[#2a2a2c] shrink-0">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <select 
                  ...
                  className="flex-1 bg-black/40 border border-[#2a2a2c] rounded-lg py-2.5 px-3 text-white focus:border-malandro-red outline-none text-sm"
                >
                ...
                <select
                  ...
                  className="bg-black/40 border border-[#2a2a2c] rounded-lg py-2.5 px-3 text-white focus:border-malandro-red outline-none text-sm"
                >
                ...
                <button 
                  ...
                  className="px-4 py-2.5 bg-malandro-red hover:bg-malandro-red-dark text-white rounded-lg text-sm font-bold flex items-center justify-center gap-2"
                >
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody>
                  {inscripciones.length > 0 ? (
                    inscripciones.map((ins, i) => (
                      <tr key={ins.id_inscripcion} className="border-b border-[#2a2a2c]/40 hover:bg-white/[0.02]">
                        <td className="py-3 pl-6">
                          <div className="flex items-center gap-3">
                            <span className="text-malandro-gray font-mono text-xs w-4">{i + 1}.</span>
                            <div>
                              <p className="text-sm font-bold text-white">{ins.clientes?.nombre}</p>
                              <p className="text-[10px] text-malandro-gray">{ins.clientes?.telefono || "Sin cel"}</p>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="py-3 pr-6 text-right text-xs text-malandro-gray">
                    ...
                    <tr>
                      <td colSpan={3} className="py-8 text-center text-sm text-malandro-gray">
            ...
            <div className="p-4 border-t border-[#2a2a2c] bg-black/20 flex justify-between items-center text-sm">
              <span className="text-malandro-gray">Total Inscritos:</span>
